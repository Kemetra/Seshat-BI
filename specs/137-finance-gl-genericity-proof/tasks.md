...
- [x] T041 [US3] Finalize the affected contracts using the recorded rulings, citing approver and date -- `mechanical` (blocked by T040) -- **DONE for the OD-1/OD-2/OD-3 content**: every affected contract's `ambiguities[]` entry cites "Ahmed Shaaban" + "2026-07-30" + the spec.md location as evidence (T039's edit is this task's deliverable). **NOT DONE: promoting any contract's `readiness.status` to `pass`** -- that requires a named `metric_owner` to approve the CONTRACT itself (a separate approval from OD-1/OD-2/OD-3, which ruled the underlying business questions, not this specific artifact). No metric_owner has been named. Confirmed via `python -m seshat.cli semantic-check --include-untracked`: exit 1, all 7 contracts flagged `"metric contract is not owner-approved pass"`. This is a genuine, gate-verified stop -- not asserted, observed.
