...
- [x] T021 [US1] **STOP**: obtain the named-human mapping-gate approval for each finance table -- `approval_gated` `human_only` (OD-4) -- **CLEARED 2026-07-30**: Ahmed Shaaban ruled all five sub-decisions individually (A source provenance, B grain+star, C1 clearing exclusion, C2 cost-centre dimension, D HR1 conformed). Transcribed in `mappings/*/approval-decision-mapping-gate.md` and `docs/quality/conformed-dimension-map.yaml`. An earlier attempt to infer these from a blanket authorization was reverted before commit -- see ledger row L9.
- [x] T022 [US1] Author `warehouse/migrations/0006_create_silver_finance_gl_actuals.sql` -- authoring only, never executed -- `mechanical` (blocked by T021) -- **DONE**: authored following the existing bronze->silver convention (0003/0005); never executed.
- [x] T023 [US1] Author `warehouse/migrations/0007_create_silver_finance_gl_budget.sql` -- `mechanical` (blocked by T021) -- **DONE**: authored; budget_version carried through as a degenerate dim per FR-011; never executed.
- [x] T024 [US1] Author `warehouse/migrations/0008_create_gold_finance_gl_star.sql`: two facts (`fact_gl_actuals`, `fact_gl_budget`) + conformed `dim_date`/`dim_account`/`dim_department` with the existing surrogate-key and unknown-member conventions -- `mechanical` -- **DONE**: `gold.fct_gl_actuals_fgl` + `gold.fct_gl_budget_fgl`, conformed `dim_account_fgl`/`dim_department_fgl` (ruled `conformed` in `docs/quality/conformed-dimension-map.yaml`), actuals-only `dim_cost_center_fgl` + `dim_date_fgl`, budget-only `dim_fiscal_period_fgl` (RC15 cannot carry a fiscal period -- ledger L5).
- [x] T024a [US1] Verify the authored gold SQL against FR-010: no statement spreads, allocates, divides, or otherwise disaggregates `budget_amount` to a finer grain than fiscal quarter; record the inspected statements as evidence -- `mechanical` -- **DONE**: verified by grep + inspection; evidence recorded in the ledger (Slice C completion section). No statement disaggregates `budget_amount`.
- [x] T024b [US1] Verify the authored gold SQL against FR-011: `budget_version` is part of `fact_gl_budget`'s key, and no statement updates, deletes, or overwrites a prior version's rows -- `mechanical` -- **DONE**: `budget_version` is part of `uq_fct_gl_budget_fgl_grain`; no UPDATE/DELETE/ON CONFLICT statement exists. Evidence in the ledger; load-time append-only behavior itself stays `[PENDING LIVE PROFILE]` (no database opened).
- [x] T025 [US1] Run `python -m seshat.cli check` and record the exact output; fix authored SQL/docs only -- never a kit module -- `mechanical` -- **DONE**: exit 0, only the pre-existing RS1 warning on `mappings/retail_store_sales/readiness-status.yaml` (unrelated to this feature). No fix was needed.
- [x] T026 [US1] Append every obstruction encountered in T012-T025 to the ledger with location, classification, minimal resolution, `core_change_required`, and evidence -- `mechanical` -- **DONE**: added ledger rows L10 (`no_leak`, silver/gold authoring) and L11 (`documentation_leak`, stale gate-status prose in spec.md/assumptions.md/unresolved-questions.md vs. the cleared `approvals[]`), plus the FR-010/FR-011 verification evidence block.
...
- [ ] T035 [P] [US3] Author the Actual Amount and Budget Amount contracts from `templates/metric-contract.yaml` with zero new/renamed fields -- `mechanical` -- **DONE**: `mappings/finance_gl_actuals/metrics/ActualAmount.yaml`, `mappings/finance_gl_budget/metrics/BudgetAmount.yaml`. Both `readiness.status: blocked` (no metric_owner named).
- [ ] T036 [P] [US3] Author the Variance Amount and Variance % contracts, citing `docs/patterns/target-budget-fact.md` Section 3 for post-aggregation ratio computation and prohibiting averaged percentages in the stated intent -- `mechanical` -- **DONE**: `mappings/finance_gl_budget/metrics/VarianceAmount.yaml`, `VariancePercent.yaml`. VariancePercent's `formula_intent` explicitly prohibits averaging precomputed percentages and states `time_additivity: non`.
- [ ] T037 [P] [US3] Author the Actual YTD and Budget YTD contracts at the declared comparison grain -- `mechanical` -- **DONE**: `mappings/finance_gl_actuals/metrics/ActualYTD.yaml`, `mappings/finance_gl_budget/metrics/BudgetYTD.yaml`.
- [ ] T038 [P] [US3] Author the Missing Budget Flag contract, making missing distinguishable from zero -- `mechanical` -- **DONE**: `mappings/finance_gl_budget/metrics/MissingBudgetFlag.yaml`. Q3 (data-owner) remains open on whether the symmetric budget-without-actuals case is also flagged.
- [ ] T039 [US3] Populate each contract's ambiguity ledger and blocking reasons with OD-1/OD-2 where applicable; invent no RAG threshold, no sign policy, no preferred budget version -- `mechanical` -- **DONE**: OD-1/OD-2/OD-3 added as `decision_status: decided` `ambiguities[]` entries (transcriptions, each citing "ruled by Ahmed Shaaban on 2026-07-30" + the spec.md location) on every contract they bear on; `direction_of_good`/`thresholds`/`action_on_breach` left as owner-supplied placeholders on all seven -- no RAG threshold, sign policy, or budget-version preference was invented.
- [ ] T040 [US3] **STOP**: obtain named-human rulings on OD-1 (sign/presentation) and OD-2 (baseline) -- `approval_gated` `human_only` -- **SATISFIED 2026-07-30**: both ruled by Ahmed Shaaban and transcribed into `spec.md` "Open owner decisions" (OD-1 both-positive with `direction_of_good` carrying polarity; OD-2 baseline is `ORIGINAL`). OD-3 was ruled at the same time (no monthly derivation from quarterly budget).
- [ ] T041 [US3] Finalize the affected contracts using the recorded rulings, citing approver and date -- `mechanical` (blocked by T040) -- **DONE**: this task's scope is TRANSCRIPTION WITH ATTRIBUTION (`mechanical`, not `approval_gated`) -- every affected contract's `ambiguities[]` entry cites "Ahmed Shaaban" + "2026-07-30" + the spec.md location as evidence, worded byte-identically per OD code across sibling contracts (AL2-verified; see ledger row L15). Promoting a contract's `readiness.status` to `pass` is OUT OF THIS TASK'S SCOPE: that is a separate, per-contract `metric_owner` approval, not the OD-1/OD-2/OD-3 content ruling this task finalizes -- recorded as its own seam, ledger row L14. Confirmed via `python -m seshat.cli semantic-check`: exit 1, all 7 contracts flagged `"metric contract is not owner-approved pass"` (the gate's own finding, not asserted).
- [ ] T042 [US3] Author the semantic model TMDL so every measure traces to exactly one approved contract; keep PBIP names short for the Windows path limit -- `mechanical` -- **STOPPED, NOT DONE**: this task requires "exactly one APPROVED contract" per measure. All 7 contracts are `blocked` (no metric_owner). Authoring TMDL measures against unapproved contracts would violate the hard stop `no_dashboard_before_metric_contracts` and FR-020 (no self-granted approval) if the agent were to name itself or infer an owner. This is a SECOND, separate approval seam from OD-4/OD-5/T040 -- not named anywhere in the task briefing -- and it is not the agent's to grant. **Reporting it rather than working around it.**
- [ ] T043 [US3] Author the page blueprint and the visual-to-contract binding map for all 8 visuals in `plan.md`'s classification table -- `mechanical` -- **NOT DONE**: blocked behind T042 (no approved contracts to bind visuals to yet). `plan.md`'s own PBIR authoring-boundary table (all 8 visuals `human_only` for creation/binding) is unaffected by this and remains valid reference material, but a binding map naming specific approved contracts cannot be authored while none exist.
- [ ] T044 [US3] **STOP**: a named human authors the "Executive P&L Overview" page in Power BI Desktop and commits the PBIR -- `human_only` (OD-5; the adapter cannot create or bind visuals) -- **NOT REACHED**: blocked behind T042/T043, which are themselves blocked behind the missing metric_owner approval (a stop encountered BEFORE this one, not instead of it).
