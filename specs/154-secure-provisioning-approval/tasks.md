...
- [ ] **T001** Confirm `contracts/provisioning-approvals.yaml` filename against
...
- [ ] **T002** Decide scope identity (R2): profile name, explicit component-id
...
- [ ] **T003** Determine how to reuse `approval_is_shape_valid` WITHOUT copying it
...
- [ ] **T004** Confirm `gitstate.is_tracked_and_clean` / `committed_text`
  semantics and import path from `seshat.integrations` without a cycle (R4).
- [ ] **T005** Decide revocation representation (R5) and how the gate reports
  `revoked` distinctly from `absent`.
- [ ] **T006** Determine the minimum extension to approval-console (F027) /
...
- [ ] **T007** Write `data-model.md`: `ProvisioningApproval`, `ApprovedScope`,
  `ApprovalVerdict` (the eight categorical outcomes), each verdict carrying a next
  action.
- [ ] **T008** Write `contracts/` refusal-reason enumeration so FR-014's
  machine-readable reasons are fixed and testable.
- [ ] **T009** Write `quickstart.md`: a human records an approval; an operator
  provisions under it. No secret in any shown output.

## Phase 2 -- The gate (TDD; US1 = the security boundary)

- [ ] **T010** [RED] `tests/unit/test_integrations_approval.py`: absent approval →
  verdict `absent`, not authorized. (FR-013)
- [ ] **T011** [RED] Approval present in worktree but UNCOMMITTED → `uncommitted`,
  not authorized. A separate test from T010, not a variant. (FR-002)
- [ ] **T012** [RED] Approval committed but on a dirty path → refused. (FR-002)
- [ ] **T013** [RED] Malformed shapes, one test each: missing decider, missing
  ...
  (FR-003, FR-004)
- [ ] **T014** [RED] Authority class other than `governance` → `wrong_authority`.
  (FR-004a)
- [ ] **T015** [RED] Unparseable YAML → `unparseable`, and NOT distinguishable in
  any way that could read as a pass. (FR-013)
- [ ] **T016** [GREEN] Implement `src/seshat/integrations/approval.py`: read-only
...
- [ ] **T017** [RED] Approved scope covering the request → `authorized`. (FR-010)
- [ ] **T018** [RED] Approved scope naming a different set → `scope_mismatch`,
  naming BOTH scopes in the reason. (FR-011)
- [ ] **T019** [RED] Capability added after the approval → not authorized.
  (FR-012)
- [ ] **T020** [RED] Superset approved scope vs subset request → authorized
  (settled in spec edge cases).
- [ ] **T021** [GREEN] Implement scope comparison per T002.

## Phase 4 -- Lifetime (standing-until-scope-change)

- [ ] **T022** [RED] Retry after partial failure, same scope → authorized, no new
  approval. (FR-012b)
- [ ] **T023** [RED] Repeat run after success, same scope → authorized; the
  approval is standing, not single-use. (FR-012a)
- [ ] **T024** [RED] Material scope change → refused pending new approval.
  (FR-012c)
- [ ] **T025** [RED] Revoked / removed / replaced → ceases to authorize.
  (FR-012d)
- [ ] **T026** [RED] Old approval, unchanged scope → age alone does NOT refuse.
  (FR-012e)
- [ ] **T027** [GREEN] Implement lifetime + revocation per T005.

## Phase 5 -- CLI seam

- [ ] **T028** [RED] `--apply --yes` with no committed approval → refused, no
  filesystem or network mutation. This is the #671 reproduction, now inverted into
  a regression test. (FR-001, FR-005)
- [ ] **T029** [RED] TTY confirmation answered yes, no committed approval →
  refused. (FR-006)
- [ ] **T030** [RED] stdin-supplied answer, no committed approval → refused.
  (FR-006)
- [ ] **T031** [RED] Valid committed approval + non-interactive run → proceeds
  without prompting; the non-interactivity flag supplies no authority. (FR-008)
- [ ] **T032** [GREEN] Rewrite `_approved()` in
...
- [ ] **T033** **CORRECT** `tests/unit/test_integrations_setup.py:321` --
...
- [ ] **T033a** [RED] Prove the `governance` authority cannot be synthesized: no
...
- [ ] **T033b** [RED] Confirm `--refresh` (network) and `--apply` (write) retain
  ...
  precondition. (FR-009)
- [ ] **T034** Prove the fail-open is closed: monkeypatch ONLY the gate to return
  `authorized` and assert the pre-fix verdict returns. Proves the gate is what
  refuses, not incidental surrounding code.
- [ ] **T035** Non-vacuity sweep: for every absence-assertion added, remove the
...
- [ ] **T036** Platform-vacuity check: no assertion may depend on a hardcoded
  platform string (e.g. `.exe`) -- CI runs Linux and such an assertion goes
  vacuous there.
- [ ] **T037** Secret-shape sweep: assert no refusal, evidence, or JSON output
  contains a credential, connection string, or token. (FR-015)
- [ ] **T038** Verification-vs-authorization: valid approval + failing provider
  verification → capability NOT reported ready, run NOT reported successful.
  (FR-016)

## Phase 7 -- Reuse and non-duplication

- [ ] **T039** Extend approval-console / approval-evidence-pack per T006 for the
  project-scoped target. No third write path. (FR-017)
- [ ] **T040** [RED] `tests/contract/test_provisioning_approval_contract.py`:
...
- [ ] **T041** Confirm catalog, resolvers, compatibility policy, installer,
  lockfile, and discovery are unchanged by diff. (FR-018)

## Phase 8 -- Gates

- [ ] **T042** `ruff format --check src/ tests/` and `ruff check src/ tests/`
  (format-check is the CI gate, not just `ruff check`).
- [ ] **T043** `pytest -m unit` full suite green. Note CI's unit job runs WITHOUT
  app extras -- do not rely on an extras-only import.
- [ ] **T044** `seshat check` exit 0 and `seshat semantic-check` as applicable.
- [ ] **T045** CodeScene: measure with `cs review` on every changed file (it is
  tokenless). Refactor any flagged function to health 10.0 rather than suppress.
- [ ] **T046** Confirm the diff contains no `plan.md`/`tasks.md`-only drift and no
  unrelated file.
- [ ] **T047** Verify **FR-020 / SC-011** are satisfied: spec 144's artifact
