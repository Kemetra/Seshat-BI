...
- [x] T001 Baseline the focused suites at the branch point and record the
...
- [x] T002 Write the failing truth-separation test first (US2): a dbt record
...
- [x] T003 Write the failing fail-closed tests (US3): corrupt JSON, missing
...
- [x] T004 Write the failing caveat tests (US1): `failed` and `blocked` records
...
- [x] T005 Write the failing prefix test (FR-017): an informational dbt caveat
      does not begin with `STOP`. Assert against `_is_stopped()` itself, not
      against a copy of its rule.
- [x] T005a Write the failing NO-SOFTENING test (FR-019/FR-020), the highest-risk
...
- [x] T006 Publish `OUTCOME_TO_EXECUTION` as a public mapping in
      `src/seshat/dbt/`, with the unknown-outcome default resolving to
      `blocked`.
- [x] T007 Point `orchestration/.../dbt_build.py` at the shared mapping and
      delete its private `_DBT_OUTCOME_TO_EXECUTION`; re-run
      `orchestration/dagster/tests` since this inverts an existing dependency.
- [x] T008 Implement the read-only classifier in a NEW sibling module, not in
...
- [x] T009 Implement the next-action caveat in `agent_next` as an ADDITIVE entry
...
- [x] T010 Assert the negative guarantees explicitly: the classifier returns no
...
- [x] T011 Update `docs/integrations/dbt-adapter.md` and
      ...
      gain no normalizer.
- [x] T012 Re-run `python scripts/export_agent_bundles.py --check` and commit any
      required regeneration if T011 changed bundled content.
- [x] T013 Run the repository gates (`pytest -m unit`, `seshat check`,
      `ruff format --check src tests`, `ruff check src tests`) and report each
      with command, exit code, and classification.
- [x] T014 Review the full diff file-by-file; every changed file must have a
