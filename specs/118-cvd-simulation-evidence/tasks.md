...
- Paths are repo-relative; single-project layout (`src/seshat`, `tests/unit`).
...
- [ ] T018 [P] Add the tool doc `docs/tools/cvd-evidence.md` mirroring the shipped `docs/tools/*.md`: what it is, how to run, the scope wall, and what it will NOT do -- names the neighbours it complements (CT1 contrast / CT2 = `design_ramp_deltae` / CT3 = `design_categorical_distinctness`, all normal-vision maths) and the DL4 design-review-evidence artifact it mirrors, and states it fills the `theme_gen.py:789` OPEN checkbox without ticking it.
