...
**Residual acceptance**: record the installed Codex version, successful
subscription login, protocol handshake, and a streamed fake-safe question on the
release platform. Protocol compatibility is tested from fixtures before that run.
...
- OpenAI Codex documentation/manual: ChatGPT subscription login, cached CLI
  authentication, noninteractive reuse, and app-server rich-client interface,
  checked 2026-08-03 from official OpenAI material.
