...
**Status history**: draft; NOT ratified. No task below may begin until a named
human records ratification in `spec.md`.
...
- [x] T001 Re-verify the dependency table in `spec.md` against the tree at
      implementation time. If a new consumer of the template has appeared, stop
      and amend the spec before continuing.
- [x] T002 Baseline the governance suites and record results, so any later
...
- [x] T003 Write failing tests for the authority: each of the four values is
      ...
      named.
- [x] T004 Write failing fail-closed tests: absent `**Status**:` line,
      unparseable line, and unreadable file each produce a named defect and
      never a pass (FR-018, FR-019).
- [x] T005 Write the failing anti-circularity test: the authority must not read
      ...
      it validates (FR-004).
- [x] T006 Implement the authority in `src/seshat/`: vocabulary, canonical case,
      line grammar, per-value evidence requirement. No state machine, no
      approval logic, no readiness import (FR-003).

## Phase 3 -- migrate consumers

- [x] T007 Migrate `tests/unit/test_spec_status_vocabulary.py` to import the
...
- [x] T008 Prove equivalent governance before touching the template. NOTE: the
...
- [x] T008a-pre Record the corpus census as a committed fixture or test
...
- [x] T008-scaffold Implement the FR-025 resolution: a Seshat-owned
...
- [x] T008a Write the failing reconciliation test FIRST: the canonical ratified
...
- [x] T008a-history Write the failing history-line regression test FIRST
...
- [x] T008b Reconcile the H3 grammar (plan §"reconciliation", option A bias:
...
- [x] T008d Reconcile `.claude/workflows/idea-to-spec.js`, the THIRD grammar
      ...
      status itself.
- [x] T008c Add the divergence contract test: if the authority's grammar and the
...
- [x] T009 Restore `.specify/templates/spec-template.md` to its `1eb0c98`
...
- [x] T010 Prove nothing was lost: with the template clean, the governance suite
      is green AND an invalid status is still rejected. This is the test that
      distinguishes a real migration from a deletion.
- [x] T011 Reconcile the `speckit.manifest.json` entry for the template using
...
- [x] T012 Correct `docs/capabilities/ownership-audit.md` lines 217-218 only:
...
- [x] T013 Add a pointer in ADR-0019 noting the policy's executable home; the
      decision itself is unchanged and is NOT superseded.
- [x] T014 Run the repository gates and report each with command, exit code, and
      classification: `pytest -m unit`, `python -m seshat.cli check`,
      `python scripts/export_agent_bundles.py --check`, `ruff format --check`,
      `ruff check`.
- [x] T015 Review the full diff file-by-file; every changed file must have a
