...
This module is the FEEDER-reading machinery behind the O1-O9 truthfulness
...
The oracle is three modules, layered so the stack stays acyclic:

* ``_capability_feeder_readers`` -- shared direct readers, no oracle deps
* ``_capability_public_ownership`` -- the spec 142/143 ownership detectors
* this module -- the O1-O8 detectors plus the ``oracle_all_clear`` aggregate

The two siblings are re-exported below, so every existing ``oracle.<name>``
caller keeps working and this module stays the single import site for tests.

ANTI-CIRCULARITY (load-bearing, repo lesson ``verifier-must-sit-on-the-risk``):
every module in this stack reads the FEEDER sources DIRECTLY and re-implements
every reader. None of them may import ``seshat.capability_feeders`` or
``seshat.capability_inventory`` -- the oracle must never learn what a feeder
says by calling the code under test, or a builder bug that hides drift on both
sides would pass vacuously. ``test_oracle_does_not_import_code_under_test``
asserts this across ALL THREE modules; a detector that moves to a sibling must
stay inside that guard's scope.
"""
...
from tests.unit._capability_public_ownership import (
    OWNERSHIP_OWNERS,
    OWNERSHIP_SURFACES,
    ownership_violations,
    public_capability_integrity_violations,
)
