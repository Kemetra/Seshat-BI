...
- **Gate status:** `CLEARED` -- mapping-gate questions Q1-Q3 were answered by Ahmed Shaaban
  on 2026-07-30 (`approval-decision-mapping-gate.md`). Model-integrity L19/L21/L22 were
  answered 2026-09-15 (A1/B1/C1; `approval-decision-model-integrity.md`). Gold Ready
  remains blocked on live `retail validate` (no DSN).
...
| L19 | Actuals are keyed to daily `dim_date_fgl`; budget is keyed to `dim_fiscal_period_fgl`. How should the two time dimensions be conformed (A1 / A2 / A3)? | Shared gold star cannot slice Actual-vs-Budget by period; `retail validate` cannot load the budget map (missing `gold_star.date_dimension`) | data-owner | (none -- Principle V; pick A1/A2/A3 in `approval-request-model-integrity.md`) | `answered` | **2026-09-15, Ahmed Shaaban (data-owner): A1.** Add `fiscal_period_sk` to `fct_gl_actuals_fgl` alongside daily `date_sk`. Transcribed from `approval-decision-model-integrity.md`; not re-decided here. |
| L21 | Does coalescing failed natural-key lookups to the gold `-1` unknown member hide D1/D2 refusals from orphan-FK checks (B1 / B2 / B3)? | A live orphan-FK PASS would be reported for the wrong reason | data-owner | (none -- Principle V; pick B1/B2/B3 in `approval-request-model-integrity.md`) | `answered` | **2026-09-15, Ahmed Shaaban (data-owner): B1.** Refuse when the source key is non-null but unmatched; COALESCE to `-1` only for null/blank keys. Transcribed from `approval-decision-model-integrity.md`; not re-decided here. |
| L22 | A valid `department_code` paired with a `cost_center_code` from a different department yields two valid FKs. How should that contradiction be handled (C1 / C2 / C3)? | No shipped check sees the pair; a department grouping disagrees with the cost-center rollup | data-owner | (none -- Principle V; pick C1/C2/C3 in `approval-request-model-integrity.md`) | `answered` | **2026-09-15, Ahmed Shaaban (data-owner): C1.** Join `dim_cost_center_fgl` on `cost_center_code` AND `department_code`. Transcribed from `approval-decision-model-integrity.md`; not re-decided here. |
