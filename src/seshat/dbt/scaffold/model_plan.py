...
def _date_attributes(raw: dict, name: str) -> tuple[str, ...]:
    """This date dimension's calendar attributes, from the SHARED resolver (#497).

    Deliberately NOT a local list. ``_DATE_COLUMNS`` used to be hardcoded here and
    silently omitted ``month_name``/``day_name``/``is_weekend``, which
    ``warehouse/migrations/0004_*.sql`` builds -- so the same approved map produced
    a structurally DIFFERENT date dimension on the two build paths, and nothing
    detected it (the parity audit compares values only, issue #492). A second
    hardcoded tuple IS that defect, so the set is resolved by
    ``star_discovery.resolve_date_attributes``, the ONE definition ``gap_detector``
    also reads. That shared resolver is what keeps the generator and the reader
    from disagreeing about a calendar, and it is why honoring a declared
    ``attributes`` key is safe here where honoring it in a single consumer was not
    (the #491 ruling).

    The resolver's ``ValueError`` is re-raised as ``ScaffoldError`` so the scaffold
    boundary keeps reporting DBT_SCAFFOLD_INVALID with the dimension named.
    """
    try:
        return _stars.resolve_date_attributes(raw)
    except _stars.CalendarContractError as exc:
        raise ScaffoldError(f"{name}: {exc}") from exc
...
def _date_dimension(inputs: _PlanInputs) -> ModelSpec | None:
    ...
    _require_dimension_prefix(name)
    surrogate = _require_identifier(raw.get("surrogate_key"), f"{name} surrogate_key")
    # No per-attribute `_require_identifier` loop here (unlike `_dimension_model`):
    # the resolver only ever returns names drawn from RC15_CALENDAR_COLUMNS, which
    # are identifiers by construction, so anything malformed is already rejected as
    # off-contract. A loop here would be unreachable and would imply this module
    # validates a set it does not own.
    attributes = _date_attributes(raw, name)
    # EVERY column keeps derivation="date_spine", including declared ones: they are
    # generated by the RC15 calendar, not mapped from bronze, so none of them gets a
    # source citation the way an entity dimension's attributes do. The surrogate
    # stays FIRST because `orchestrator._dimension_sql` dispatches the date-spine
    # renderer on `columns[0].derivation`.
    columns = tuple(
