...
def _add_date_attributes(
    ...
    A date dimension's attributes are GENERATED by the RC15 calendar, so there is
    no source column for a ``columns[].gold_placement`` to point at -- they come
    from ``star_discovery.resolve_date_attributes``, the ONE shared resolver the
    dbt scaffolder builds from too (issue #497). Reading the same resolver as the
    generator is what stops this reader from reporting a column "available" that no
    build path produces: when a map NARROWS its calendar via ``attributes``, the
    resolver narrows both surfaces together. Using a fixed set here while the
    builder honored the map would produce exactly that false availability.

    The surrogate key IS declared per-map, so it is read from the map.

    An off-contract attribute raises ``CalendarContractError`` in the resolver. This
    reader is an ADVISORY surface (it reports gaps, it is not the scaffold gate), so
    it must not explode on a malformed map: an unresolvable calendar contributes no
    attribute tokens, which keeps a bound column reported missing (fail CLOSED) and
    leaves the loud, actionable error to ``seshat dbt scaffold``.
    """
    from seshat.star_discovery import CalendarContractError, resolve_date_attributes

    try:
        attributes = resolve_date_attributes(date_dim)
    except CalendarContractError:
        attributes = ()
    dims.update(attributes)
    cols.update(attributes)
