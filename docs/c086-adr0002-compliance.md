...
| **D13** | Silver as TABLE via **idempotent numbered migration**; transform order load-bearing | **PASS (resolved)** | `0001`/`0002` ARE transaction-idempotent (DROP+CREATE in BEGIN/COMMIT, numbered `NNNN_`); transform order correct. The 22 S4b warnings were the **rule** being wrong (blanket guarded-form), not the SQL — DROP+CREATE is the *correct* idempotency for rebuildable marts. **RESOLVED:** S4b is now layer-aware (commits `3d8443e`/`394f1ce`/`c75f5ba` on `fix/s4b-layer-aware`) — silver/gold DROP+CREATE-in-txn passes; bronze bare DROP/CREATE = ERROR; fail-closed on unknown schema. `retail check .` now shows **0 S4b findings** on C086. | No | **DONE** — S4b is the checker rule; refined to layer-aware. See §6 of the worked example. |
...
- **PASS: 16/16** — all defaults satisfied. 12 static-PASS (D1,D3–D12,D14) + D15 + the 3 live-confirmed
  (D2, D15-coverage, D16).
- **WARN: 0** (D13's bare-DDL-vs-S4b nuance is now **resolved** — S4b made layer-aware; see below).
- **FAIL: 0**

**D13/S4b update (2026-06-24):** resolved by making S4b layer-aware (silver/gold DROP+CREATE-in-txn
