...
rules on 0.9.0. No existing verb's flag, output shape, or exit-code meaning
changed either, so nothing here reaches the MAJOR row.

Per the policy, ambiguity resolves toward the larger bump -- but the ambiguity
here was resolved by measurement, not by preference, and this is 0.9.0 rather
than 0.8.3 on the strength of the new console script alone.
