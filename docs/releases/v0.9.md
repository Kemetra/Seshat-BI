...
- **The Codex bridge has no production caller.** `CodexBridge` and its `health()`
  ...
  endpoint drives them. This is tracked as issue #618 and is **not** closed by
  this release. Do not read "Codex integration" into this release as a working
  user-facing feature.
...
This candidate carries **86 non-merge commits across 31 referenced pull
requests** -- 31 `feat`, 26 `fix`, 14 `docs`, 10 `refactor`, and the remainder
`build`/`test`/`style`/`ci`.
...
Two additive surfaces put this on the MINOR row of
...
- **New `seshat check` rule coverage.** The rule set grows; a consumer pinned to
  0.8.2 keeps passing until they touch a newly-covered surface. That is the
  policy's "NEW rule is registered" row.

No existing rule id's predicate was broadened and no existing verb's flag,
output shape, or exit-code meaning changed, so nothing here reaches the MAJOR
row. Per the policy, ambiguity resolves toward the larger bump -- the additive
rows govern, and this is 0.9.0 rather than 0.8.3.
