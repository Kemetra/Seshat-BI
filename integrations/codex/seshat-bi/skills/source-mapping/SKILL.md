...
> column's own keep decision. See the medallion playbook, Phase 2, for the rationale.
...
- Gate + principles: the kit architecture note, Sec 5; constitution
  Principles IV, V.
- Method / defaults: the medallion playbook; the retail cleaning-defaults
  decision record (RC1-RC16).
- Live half (after silver/gold exist): the `retail-validate` skill.
- A filled instance: a filled worked example (an
