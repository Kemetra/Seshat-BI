...
Seed/update the per-table status from the readiness-status template at the
...
instance (see the filled worked examples), never copied.

## See also

- The two stages spanned: the Source Ready and Mapping Ready definitions.
- The reviewable definition-of-done: the onboarding checklist.
- The spine + state model: the readiness model and readiness pipeline, plus the
  readiness-status template.
- The delegated mapping leg: the `source-mapping` skill.
- The downstream build verb (out of scope here): the `retail-build-warehouse`
  skill.
- The roadmap row: F006, Layers 1-2, Source -> Mapping.
- Principles: constitution I, IV, V, VII, VIII, IX.
