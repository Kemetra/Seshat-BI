...
- **Applying the silver/gold SQL to the DB** -- authoring the `warehouse/` `.sql` is
  NO LONGER deferred: it shipped as F006, and
...
- The verbs: the `retail-onboard-table`, `source-mapping`,
  `grain-confidence-reviewer`, `retail-govern`, `retail-validate`,
  `retail-semantic-check` and `pbip-workflow` skills.
- The posture: constitution Principles I, IV, V, VIII.
- The method + exit gates: the medallion playbook.
- The replay reference: a filled worked example.
